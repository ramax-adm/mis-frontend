...
export async function GetProductClassificationTypes() {
  const response = await GetFetch(
    urls.SENSATTA.GET_PRODUCT_CLASSIFICATION_TYPES
  );
...
export async function GetMarkets() {
  const response = await GetFetch(urls.SENSATTA.GET_MARKETS);
...
export async function PostSyncStockBalanceWithSensatta() {
  const response = await PostFetch(urls.SENSATTA.POST_SYNC_STOCK_BALANCE);
