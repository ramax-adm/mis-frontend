...
import { DeleteFetch, GetFetch, PatchFetch, PostFetch, urls } from '../axios/api-base'
...
export const PostAddUserCompany = async ({
  ...
  const response = await PostFetch(urls.USER.POST_ADD_USER_COMPANY, { userId, companyCode })
...
export const PostAddUserWebpage = async ({
  ...
  const response = await PostFetch(urls.USER.POST_ADD_USER_APP_WEBPAGE, { userId, pageId })
  return response.data
}

export const DeleteUserAppWebpage = async ({ id }: { id: string }) => {
  const response = await DeleteFetch(urls.USER.DELETE_USER_APP_WEBPAGE.concat(`/${id}`), { id })
  return response.data
}

export const DeleteUserCompany = async ({ id }: { id: string }) => {
  const response = await DeleteFetch(urls.USER.DELETE_USER_COMPANY.concat(`/${id}`), { id })
  return response.data
}
