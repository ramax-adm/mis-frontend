...
interface HistoryAbsenceHoursByDepartmentGraphProps {
  data: HistoryAbsenceHoursByDepartmentItem[]
}
export function HistoryAbsenceHoursByDepartmentGraph({
  data,
}: HistoryAbsenceHoursByDepartmentGraphProps) {
      ...
      >
        ...
        {departments.map((department, index) => (
          <Area
            key={department}
            type='monotone'
            dataKey={`${department}.absenceHoursInSeconds`}
            dot={{ r: 1 }}
            activeDot={{ r: 4 }}
...
const getData = ({ data }: HistoryAbsenceHoursByDepartmentGraphProps) => {
  const response: ({
    date: string
  } & {
    [department: string]: { absenceHours: string; absenceHoursInSeconds: number } | string
  })[] = []
  ...
  data.forEach((item) => {
    ...
    currentGroup[item.department] = {
      ...
      absenceHoursInSeconds: item.absenceHoursInSeconds,
    }
  })
