...
export function SyncedFilesTable({ dateSelected, entitySelected }: SyncedFilesTableProps) {
  const { data: syncedFiles = [], isFetching } = useGetSyncedFiles({
    date: dateSelected,
    entity: entitySelected,
  })
  const { mutateAsync: downloadSyncedFile, isPending: isDownloadingSyncedFile } =
    useDownloadSyncedFile()

  const handleDownloadSyncedFile = async (id: string) => await downloadSyncedFile({ id })

  const columns = getColumns({ handleDownloadSyncedFile, isDownloadingSyncedFile })
