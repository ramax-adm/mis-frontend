...
import { InvoiceTraceabilityReinvoicingsByInvoiceTable } from "../tables/invoice-traceability-reinvoicings-by-invoice-table";
import { toLocaleString } from "@/utils/number.utils";
import { GetBusinessAuditSalesInvoiceTotals } from "@/types/api/business-audit";

export function InvoiceTraceabilityReinvoicingsByInvoiceCard() {
  ...
  const haveSomeData =
    data?.reInvoicings && Object.values(data.reInvoicings.data).length > 0;
  return (
    <BusinessAuditCustomizedCard cardTitle='Notas Fiscais - Refaturamentos'>
      {!haveSomeData && !isFetching ? (
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
          <CardTotals totals={data?.reInvoicings.totals} />
          <InvoiceTraceabilityReinvoicingsByInvoiceTable
            data={data?.reInvoicings.data}
