...
interface AccountReceivableExpiredBucketsGraphProps {
  data?: Record<string, AccountReceivableResumeAgg>;
  isFetching?: boolean;
}
export function AccountReceivableExpiredBucketsGraph({
  data = {},
  isFetching,
}: AccountReceivableExpiredBucketsGraphProps) {
...
const getData = ({
  ...
  return Object.entries(data)
    .filter(([key]) =>
      expiredSituations.includes(key as AccountReceivableBucketSituationEnum)
    )
    .map(([key, value]) => ({
      ...value,
    ...
    .sort(
      (a, b) =>
        expiredSituations.indexOf(
          undoReplaceBucketSituation(a.bucketSituation)
        ) -
        expiredSituations.indexOf(undoReplaceBucketSituation(b.bucketSituation))
    );
};
