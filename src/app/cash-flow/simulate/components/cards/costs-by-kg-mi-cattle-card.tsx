...
interface CostsByKgMiCattleCardProps {
  data: Pick<ProjectCostByKgResponse, "costs" | "total">;
}
export function CostsByKgMiCattleCard({ data }: CostsByKgMiCattleCardProps) {
  const columns = getColumns();
  return (
    <Card.Root sx={{ height: "150px", padding: 1, gap: 1 }}>
      <Card.Title>Custo Animais R$/KG MI</Card.Title>
      <Card.Content sx={{ height: "100%" }}>
        <CustomizedTable<{ label: string; value: number; costByKg: number }>
          ...
          columns={columns}
          data={data.costs.custoMiAnimais}
        />
      </Card.Content>
      <Card.Footer sx={{ display: "flex", flexDirection: "row", gap: 2 }}>
        ...
        >
          TOTAL: {data.total.totalCustoAnimaisMi}
        </Typography>
        <Typography
          fontWeight={700}
          fontSize={"12px"}
          sx={{ color: COLORS.TEXTO }}
        >
          RS/KG: {data.total.totalCustoKgAnimaisMi}
        </Typography>
      </Card.Footer>
    </Card.Root>
  );
