...
const COLORS = ['#BE1717', '#E65100', '#1B5E20']
...
const getData = ({ data }: QuantityFreightsByStatusCardProps) => {
  return [
    {
      name: 'ABERTO',
      value: data.quantityActive,
    },
    {
      name: 'S/FRETE',
      value: data.quantityNoFreight,
    },
    {
      name: 'FECHADO',
      value: data.quantityClosed,
    },
  ]
}
...
const renderLegend = (value: string, entry: any) => {
  const map = new Map()
  map.set('FECHADO', `FECHADO`)
  map.set('ABERTO', `ABERTO`)
  map.set('S/FRETE', `S/FRETE`)
