  ...
  function createNewData(column: Column<T>, value: T) {
    ...
    if (column?.value?.first?.parent) {
      ...
      if (parent?.length > 0) {
        ...
        if (
          (firstParent as Record<string, string>)?.[
            `${column.value.first?.value}`
          ] !== null
        )
          return formatValue(
            column.type,
    ...
    if (column?.value?.second) {
      ...
      if (
        (secondParent as Record<string, string>)?.[
          `${column.value.second.value}`
        ]
      )
        return formatValue(
          column.type,
    ...
    if (column.type === "bool") {
      return value?.[`${column.value.first?.value}`]
        ? column.trueMessage
        : column.falseMessage;
    }
