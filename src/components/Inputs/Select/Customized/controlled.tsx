  ...
  return (
    <FormControl fullWidth sx={{ margin: 0, ...sx }} size={size}>
      <Autocomplete
        ...
        disabled={disabled}
        options={options || []}
        renderOption={renderOption ? (props, option) => renderOption(props, option) : undefined}
        onChange={(_, data: SelectOption<TValue> | SelectOption<TValue>[] | null) => {
          if (!data) {
        ...
        renderInput={(params) => (
          <TextField {...params} name={name} label={label} helperText={error && errorMessage} />
        )}
      />
    </FormControl>
  )
